...
return new class extends Migration {
    ...
    {
        Schema::create('department_picking_detail', function (Blueprint $table) {
            $table->id();
            $table->string('key')->comment('单据号');
            $table->date('date')->comment('单据日期');
            $table->integer('department_picking_id')->comment('领料单ID');
    ...
    {
        Schema::dropIfExists('department_picking_detail');
    }
};

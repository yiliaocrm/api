...
return new class extends Migration {
    ...
    {
        Schema::create('inventory_transfer_detail', function (Blueprint $table) {
            $table->id();
            $table->string('key')->comment('单据号');
            $table->date('date')->comment('单据日期');
            $table->tinyInteger('status')->default(1)->comment('状态 1：草稿、2：审核通过');
            $table->integer('inventory_transfer_id')->comment('调拨主单ID');
    ...
    {
        Schema::dropIfExists('inventory_transfer_detail');
    }
};

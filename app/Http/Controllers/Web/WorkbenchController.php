...
class WorkbenchController extends Controller
{
     ...
     * 咨询管理
     */
    public function consultant(WorkbenchRequest $request): JsonResponse
    {
        $rows = $request->input('rows', 10);
        $sort = $request->input('sort', 'created_at');
        $order = $request->input('order', 'desc');
        $keyword = $request->input('keyword');
        $createdAt = user()->hasAnyAccess(['superuser', 'workbench.consultant.history'])
        ...
        $query = Consultant::query()
            ...
            ->queryConditions('WorkbenchConsultant')
            ->when($keyword, fn (Builder $query) => $query->whereLike('customer.keyword', '%'.$keyword.'%'))
            // 权限限制
